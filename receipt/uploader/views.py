...
class ReceiptView(APIView):
    ...
    def get(self, request):
        ...
        ret_count, num_pages, files = service.get_receipts(bucket, sort, start, limit, searchTerm)
    ...
    def post(self, request):
        ...
        if receipt_inst:
            ...
            try:
                ReceiptProcessorThread(receipt_inst.file, receipt_inst.pk).start()
            except Exception:
...
class ReceiptReParseView(APIView):
    permission_classes = (IsAuthenticated,)
    def post(self, request):
        data_inst = ReceiptReParseSerializer(data=request.data, user=request.user)
...
class ReceiptDeleteView(APIView):
    ...
    def post(self, request):
        data_inst = ReceiptDeleteSerializer(data=request.data, user=request.user)
        if data_inst.is_valid():
